// Gfx/EmitterMeshRenderer.cpp — GPU upload + drawing of a .MOBJECT2 effect mesh.
//
// Faithful reproduction of `Mesh_DrawAnimatedFrame 0x430BE0` (decompiled + disassembled on 2026-07-17,
// idaTs2, TwelveSky2.exe, imagebase 0x400000). Original caller: Mesh_DrawInstancesLOD 0x431A90.
// Each block carries its EA anchor. STANDALONE MODULE: device supplied by the caller, no wiring.
//
// Runtime offset correspondence (a3 = 268-byte mesh) -> C++ fields (see also EmitterMeshRenderer.h):
//   a3+0   type (gate)            a3+4   compression mode (==1 => CPU ; we are always GPU/MANAGED)
...
//   a3+88  frameBbox (40·N)       a3+92  frameScale (4·N) a3+96..176 header2/billboard template
...
// Safe little-endian reads from a blob (native x86). Return 0 out of bounds.
...
// Writes 3 floats (xyz) at `off` into a vertex buffer (billboard quad corners).
...
// Binary's 0.01 constant (dbl_7EDB40 = 0x3F847AE140000000 ≈ 0.009999999776482582).
constexpr double kHundredth = 0.009999999776482582;

// GXD_SetDirectionalLight 0x403980: fills a directional D3DLIGHT9 and sets light 0.
//   mode 2 (glow)  : Ambient = (r,g,b,a)                         (a5=1.0 on the draw side)
//   mode 1 (reset) : Ambient = scene ambient (a1+1124.. ; fallback provided standalone)
...
//  QPC timer — v50 = (count - start)/freq + phase  (dbl_18C4F80 freq / dbl_18C4F88 start)
double EmitterMeshRenderer::ElapsedSeconds() const {
    ...
    if (!timerInit_) {
        ...
        startCount_ = now.QuadPart;            // local origin (relative phase — faithful to the intent)
        timerInit_  = true;
    }
...
bool EmitterMeshRenderer::uploadTexture(IDirect3DDevice9* dev, const asset::SObjectTexture& src,
    ...
    if (!src.present || src.packedSize == 0 || src.compressed.empty() ||
        ...
        return false;                          // missing texture (Tex_ReadPacked gate `if(a1[1])`)
    }
    auto& zlib = asset::Zlib::Instance();
    if (!zlib.Available()) return false;
    // Decompressed block = [ddsSize DDS bytes][u32 procMode][u32 alphaMode] (= rawSize) — cf. MeshRenderer.
...
bool EmitterMeshRenderer::uploadMesh(IDirect3DDevice9* dev, const asset::Mobject2Mesh& src,
    ...
    if (src.empty || src.type == 0) {          // empty mesh (type==0) — kept but not drawn
        out.valid = false;
        return true;
    }
    out.valid      = true;
    out.frameCount = (src.n == 0) ? 1u : src.n; // N = a1[21] (frame count; guard minimum 1)

    // --- Material header decoding (header1, 76 o = a1[2..20]) ---
    // header1 offset = (mesh offset) - 8  (a1[2] = mesh+8 = header1[0]).
    ...
    out.glowFrom[2]       = RdI32(h, 32); // mesh+40   (header1[9] @ byte 36 = mesh+44 : unused)
    out.glowTo[0]         = RdI32(h, 40); // mesh+48
    out.glowTo[1]         = RdI32(h, 44); // mesh+52
    out.glowTo[2]         = RdI32(h, 48); // mesh+56   (header1[13] @ byte 52 = mesh+60 : unused)
    ...
    // Per-frame tables + billboard template (copied raw, indexed at draw time).
    ...
    // --- Textures: main (a1[51]) + extras (a1[66], 56·extraTexCount) ---
    ...
    for (const asset::SObjectTexture& t : src.extraTex) {
        ...
        uploadTexture(dev, t, holder);             // 56 o each @0x430B22
...
void EmitterMeshRenderer::ReleaseCache() {
    cache_.clear();       // EmitterGpuObject/Mesh/Subset: destructors release VB/IB/GpuTexture
    loadFailed_.clear();
}

EmitterMeshRenderer::~EmitterMeshRenderer() {
    // RAII: destroying cache_ releases each EmitterGpuSubset (VB/IB) and EmitterTexHolder
    // (GpuTexture). Nothing to do manually.
}

//  DrawMesh — byte-exact reproduction of Mesh_DrawAnimatedFrame 0x430BE0
...
void EmitterMeshRenderer::DrawMesh(IDirect3DDevice9* dev, const EmitterGpuMesh& m,
    ...
    // v51 = v10 = ftol(a5): frame index (truncation toward zero).                    @0x430C06
    int frameI = static_cast<int>(args.frame);
    // Timer v50 = elapsed seconds + a7.                                              @0x430C43
    const double t = ElapsedSeconds() + static_cast<double>(args.timePhase);

    // --- Texture holder selection (v43) --------------------------------------------------------
    const EmitterTexHolder* holder = args.overrideTex; // ecx (a2): override if non-null
    if (m.animatedTex == 1 && !m.extraTex.empty()) {   // animated texture @0x430C66
        const int cnt = static_cast<int>(m.extraTex.size());
        int idx;
        if (m.texMinFrame < 1) {                       // TEMPORAL index @0x430C72
            // idx = ((int)(animTexSpeed · t · 0.01)) % extraTexCount
            int ti = static_cast<int>(static_cast<double>(m.animTexSpeed) * t * kHundredth);
            idx = ti % cnt;                            // idiv -> signed remainder
            if (idx < 0) idx += cnt;                   // defensive bound (array access)
        } else {                                       // mapped onto [min,max] @0x430C92
            ...
            if (idx < 0) idx = 0;                       // defensive bound
            if (idx >= cnt) idx = cnt - 1;
        }
        holder = &m.extraTex[idx];                     // v43 = extraTex[idx] @0x430CE2
    }
    if (!holder) holder = &m.mainTex;                  // default = mainTex (a3+204)
    ...
    if (args.pass == 1) {                              // @0x430CE6
        if (blendMode == 2) return;                    // opaque pass: skips additive @0x430CFD
    } else {
        if (blendMode != 2) return;                    // additive pass: additive only @0x430D17
        // alpha = frameScale[frame] · (255 - alpha)   (truncation) @0x430D20..0x430D53
        const float sc = RdF32(m.frameScale, static_cast<size_t>(frameI) * 4);
        const int scaled = static_cast<int>(static_cast<double>(sc) *
                                            static_cast<double>(255 - alpha)); // truncating (ftol)
        alpha = static_cast<uint8_t>(scaled);
    }

    // --- Subset selection (LOD by face count, a6 factor) --------------------------------
    ...
    int i = 0;                                           // loop @0x430DAC
    for (; i < nSub; ++i)
        if (result >= static_cast<int>(m.subsets[i].faceCount)) break;
    if (i == nSub) return;                               // no LOD fits @0x430DBA
    const EmitterGpuSubset& sub = m.subsets[i];

    // Frame index safety (the flipbook guarantees 0<=frame<N on the caller side; we clamp
    // to avoid any out-of-bounds VB/frameBbox access when standalone). Not present in the binary.
    ...
    // --- Fade states BEFORE drawing (v40 / v7) ----------------------------------------------
    ...
    if (blendMode == 1) {                              // @0x430E59
        ...
        if (alpha != 0) { blendCommon(static_cast<DWORD>(alpha) << 24); tssTail(); } // else -> LABEL_33
        ...
        // blendMode 0 & alpha 0: NO state set (LABEL_34 direct).
    }

    // --- Glow: animated directional light (from/to ping-pong) -----------------------------
    if (m.glowEnable == 1) {                           // @0x430F9C
        ...
        for (int j = 0; j < 3; ++j) {
            ...
            if (range <= 0.0) {
                ...
                const int    k    = static_cast<int>(tt); // v42 = ftol(tt) (truncation)
                const double frac = range * (tt - static_cast<double>(k)); // v71
                const bool even = (k & 1) == 0;        // 0x80000001 parity idiom @0x430FFF..0x431018
    ...
    } else {                                           // reset: mode 1 (scene ambient — fallback) @0x4310ED
        SetGxdLight(dev, args.sceneAmbient[0], args.sceneAmbient[1], args.sceneAmbient[2], 1.0f);
    }

    // --- UV-scroll: animated texture matrix ------------------------------------------------
    if (m.uvEnable == 1) {                             // @0x4310FA
        ...
        tm._31 = u; tm._32 = v;                        // UV translation (v60=_31, v61=_32)
        dev->SetTransform(D3DTS_TEXTURE0, &tm);        // 16 @0x4311E9
    }

    // --- Drawing: billboard (camera-facing quad) or geometry ----------------------------------
    if (m.billboardEnable == 1 &&                      // @0x4311EE
        ...
        const D3DXVECTOR3 centerMesh(RdF32(m.frameBbox, fb + 24), RdF32(m.frameBbox, fb + 28),
                                     RdF32(m.frameBbox, fb + 32)); // v32+6 = center (offset+24)
        ...
        // Template = header2 (baked UVs); we rewrite the xyz of the 4 corners (stride 20). @0x431286..0x431378
        uint8_t quad[80];
        std::memcpy(quad, m.billboardTemplate.data(), 80);
        WriteXYZ(quad,  0, c.x - right.x * halfW - up.x * halfH,  // corner 0: c - right·W - up·H
                           c.y - right.y * halfW - up.y * halfH,
                           c.z - right.z * halfW - up.z * halfH);
        WriteXYZ(quad, 20, c.x - right.x * halfW + up.x * halfH,  // corner 1: c - right·W + up·H
                           c.y - right.y * halfW + up.y * halfH,
                           c.z - right.z * halfW + up.z * halfH);
        WriteXYZ(quad, 40, c.x + right.x * halfW - up.x * halfH,  // corner 2: c + right·W - up·H
                           c.y + right.y * halfW - up.y * halfH,
                           c.z + right.z * halfW - up.z * halfH);
        WriteXYZ(quad, 60, c.x + right.x * halfW + up.x * halfH,  // corner 3: c + right·W + up·H
                           c.y + right.y * halfW + up.y * halfH,
                           c.z + right.z * halfW + up.z * halfH);

        D3DXMATRIX id; D3DXMatrixIdentity(&id);
        dev->SetTransform(D3DTS_WORLD, &id);           // 256, identity (corners already in world space) @0x4313D3
        ...
        // GXD geometry path (a3+4 != 1: device VB/IB). We always upload as MANAGED -> this path.
        // (The CPU path a3+4==1 / DrawIndexedPrimitiveUP @0x4314BE is not taken: no system
        //  buffer. Documented: our buffers are GPU.)
        if (sub.vb && sub.ib) {
    ...
    SetGxdLight(dev, args.sceneAmbient[0], args.sceneAmbient[1], args.sceneAmbient[2], 1.0f); // SetLight(0, dword_18C5358) @0x431620 — fallback: reconstructed scene light

    // Fade teardown per blendMode (mirrors @0x431626..0x4317B1).
    ...
    if (blendMode == 0) {
        if (alpha == 0) return;                        // nothing set -> nothing to restore @0x431633
        restoreOpaque();
    } else if (blendMode == 2) {
        restoreOpaque();
    } else {                                           // blendMode 1
