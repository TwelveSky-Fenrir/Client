// Gfx/ZoneFxEmitter.cpp — implémentation FIDÈLE du moteur de particules « Object A » (.WP).
//
// Vérité IDA : TwelveSky2.exe (imagebase 0x400000). Chaque bloc cite son ancre (nom + 0xADDR).
// Voir ZoneFxEmitter.h pour la carte des structures et Docs/TS2_EXTRACT_WP_EMITTERS.md pour la
// spec octet-exacte. N'utilise que le SDK Windows/Direct3D9 + d3dx9 (comme le binaire).
...
// π/180 — littéral exact du binaire (flt_7BB26C = 0x3C8EFA35 = 0.017453292), lu par UpdateEmit
// @0x6a7947/@0x6a7961/@0x6a797b (degrés -> radians des rotations d'origine).
static constexpr float kDeg2Rad = 0.017453292f;
// ±100 — bornes de direction aléatoire (0xC2C80000 / 0x42C80000 poussées @0x6a7a9e/@0x6a7a99).
static constexpr float kRandLo = -100.0f;
static constexpr float kRandHi =  100.0f;
// ±0.5 — demi-boîte (flt_7A939C = 0x3F000000 = 0.5 ; flt_7BB294 = 0xBF000000 = -0.5) des cas 5/6.
static constexpr float kBoxHalfPos =  0.5f;
static constexpr float kBoxHalfNeg = -0.5f;
// 1.0 — seuil d'émission (flt_7ED9B0 = 0x3F800000), comparé à spawnAccum @0x6a7877/@0x6a8001.
static constexpr float kSpawnThreshold = 1.0f;

// Gate global (miroir dword_800080 != 0). Le module possède son scratch -> prêt par défaut.
static bool s_zoneFxReady = true;
void ZoneFx_SetReady(bool ready) { s_zoneFxReady = ready; }
bool ZoneFx_Ready()              { return s_zoneFxReady; }

// ===========================================================================================
//  ZoneFx_BuildTemplate — remplit le template 232o depuis le blob disque asset::FxNode.
//  Ancre : Fx_NodeLoadFromHandle 0x6A69F0 (this+0=enabled=1 ; texture à +52 ; 18 ReadFile
//  remplissant [+72,+216) = les 144 o de asset::FxNode.fields, recopiés tels quels ici).
// ===========================================================================================
void ZoneFx_BuildTemplate(FxEmitterTemplate* tmpl, const uint8_t* diskFields, size_t diskFieldsSize,
    ...
    tmpl->enabled = 1;              // *this = 1 pour un nœud chargé (Fx_NodeLoadFromHandle @0x6a6cf4)
    tmpl->texture = texture;        // GPU tex à +52 (SetTexture(0,*(tmpl+52)) @0x6a745e)
    // Chemin keyframe NON porté : frameTrack/frameCount/frameMatrices restent nuls -> la matrice de
    // frame de UpdateEmit prend l'else-branch identité (@0x6a78c3), fidèle au cas ambiant sans
    // animation de transform d'émetteur. TODO ancre 0x6a787d : baker la piste quaternion (this+14).
...
//  Borne les champs (indices float du binaire = this+N -> octet 4N) puis dérive colorRate.
// ===========================================================================================
void ZoneFx_ComputeGradients(FxEmitterTemplate* t) {
    ...
    // start/endColor bornés [0,255] (this+35..42 = +140..+168) @0x6a6de8..@0x6a6f67
    for (int i = 0; i < 4; ++i) {
...
// ===========================================================================================
//  ZoneFx_Init — Particle_Init 0x6A7020. Alloue le pool à la 1re frame.
// ===========================================================================================
void ZoneFx_Init(FxParticlePool* pool, FxEmitterTemplate* t) {
    ...
    // Zéro-init du SEUL champ `alive` (1er DWORD) de chaque slot — fidèle : le binaire n'écrit que
    // *(v8+particles)=0 par pas de 56 (@0x6a7093), pas les 56 o entiers. Le spawn remplit tout le
    // reste avant de poser alive=1, donc les champs non init ne sont jamais lus vivants.
...
// ===========================================================================================
//  ZoneFx_UpdateEmit — Particle_UpdateEmit 0x6A7530. Intégration + émission (switch 6 formes).
//
//  ⚠ FIDÉLITÉ DES FORMES 2 ET 6 (dégénérées dans le binaire) : les cas 2 et 6 lisent un scratch
//    PERSISTANT (fenêtre wC = var_358/354/350) qui n'est pas fraîchement (ré)initialisé à chaque
//    spawn — le binaire réutilise des restes de l'itération précédente / du vecteur de force
//    (probable BUG d'origine : le cas 2 normalise wA puis transforme wC, le cas 6 n'écrit que
//    wC.x via le chevauchement wB.z↔wC.x). On REPRODUIT ce comportement en gardant les fenêtres de
//    scratch en variables locales PERSISTANTES sur toute la boucle de spawn (miroir de la pile).
//    Les cas 1/3/4/5 sont bien définis. Ancre : disasm 0x6a7a63-0x6a8014.
//
//  ⚠ MATRICES DÉCALÉES : le binaire passe la matrice monde M à des offsets OCTETS (M+0, M+8, M+20)
//    selon la forme (probable aliasing de pile). On reproduit à l'IDENTIQUE via arithmétique de
//    pointeur dans un scratch de 2 matrices contiguës [M, frame·Rz] — exactement la disposition de
//    pile du binaire (var_200 suivi de var_1C0=frame·Rz) : les lectures décalées y restent valides
//    ET lisent les mêmes octets adjacents. M+8 (D3DXVec3TransformNormal, cas 1/2/3/5/6) et M+20
//    (D3DXVec3TransformCoord, cas 2/6). Ancre : 0x6a7e33/0x6a7f8a/0x6a7f3f (var_200/var_1F8/var_1EC).
// ===========================================================================================
void ZoneFx_UpdateEmit(FxParticlePool* pool, float dt, const float pos[3], const float rot[3],
    ...
    // Cull frustum de l'origine (Cam_FrustumTestPoint6 @0x6a75c2) : hors frustum -> rien.
    if (frustum && !frustum(pool->position)) return;

    // Vecteur de force calculé UNE FOIS par update : force = Rand(forceRandMin, forceRandMax) + forceBase
    // (v85/v86/v87 @0x6a75f7/@0x6a7617/@0x6a7639). Ces slots (s350/s34C/s348) chevauchent le scratch
    // de spawn (wC.z / etc.) — on les garde donc dans les variables persistantes ci-dessous.
    ...
    // ---- Intégration des particules vivantes (stride 56) — @0x6a7660-0x6a7814 ----
    int live = 0;
    ...
    // ---- Émission — @0x6a781e-0x6a8043 ----
    // Gate durée d'émission (@0x6a783c) : émet si emissionDuration<=0 (continu) OU elapsed<=durée.
    ...
    // Matrice monde d'émission M = frame · Rz · Ry · Rx · T (ordre EXACT @0x6a793f-0x6a7a55).
    // `frame` = identité (else-branch @0x6a78c3 ; chemin keyframe template+56 non porté).
    ...
    // Scratch de 2 matrices CONTIGUËS : mtx[0]=M (var_200), mtx[1]=frame·Rz (var_1C0=v95). Cette
    // disposition reproduit la pile du binaire pour les lectures décalées M+8/M+20 (voir bandeau).
    D3DXMATRIX mtx[2];
    D3DXMATRIX m98, m97;
    D3DXMatrixMultiply(&mtx[1], &frame, &Rz); // v95 = frame·Rz @0x6a79a7 (= var_1C0, données adjacentes)
    ...
    const D3DXMATRIX* Mp8   = reinterpret_cast<const D3DXMATRIX*>(mbase + 2);             // M+8 octets
    const D3DXMATRIX* Mp20  = reinterpret_cast<const D3DXMATRIX*>(mbase + 5);             // M+20 octets

    // Fenêtres de scratch PERSISTANTES (miroir des var_XXX de pile ; wB.z ≡ wC.x = s358). Init au
    // pré-boucle du binaire : var_354=0 (@0x6a7a57), var_350/34C/348 = vecteur de force (ci-dessus).
    // var_358/364/368/36C/360/35C = pile non initialisée -> 0 (choix build-safe non inventif, cf.
    // bandeau formes 2/6 ; ces slots ne sont lus que par les formes dégénérées).
    float s36C = 0.0f, s368 = 0.0f, s364 = 0.0f;   // fenêtre A (var_36C/368/364)
    float s360 = 0.0f, s35C = 0.0f, s358 = 0.0f;   // fenêtre B (var_360/35C/358) ; s358 = wC.x
    float s354 = 0.0f;                             // wC.y (var_354) — mis à 0 @0x6a7a57
    // (s350 déjà déclaré = wC.z / force.x)

    for (int slot = 0; slot < pool->particleCount; ++slot) {  // v84 < particleCount @0x6a8014
        Particle& p = pool->particles[slot];
        if (p.alive) continue;                                // if(!*v31) : seulement les slots libres @0x6a7a66
        const int shape = Crt_ftol(t->shape);                 // switch(Crt_Dbl2Uint(t+84)) @0x6a7a7f
        switch (shape) {
            case 1: { // sphère surface, vitesse = speed (@0x6a7a90)
            ...
            case 3: { // sphère volume, vitesse = Rand(0,speed) (@0x6a7bcb)
                p.alive = 1; p.age = 0.0f;
            ...
            case 4: { // sphère + vitesse indépendante (transforms via M, forme bien définie) (@0x6a7c44)
                p.alive = 1; p.age = 0.0f;
                ...
                s360 = s36C; s35C = s368; s358 = s364;         // wB = wA (dir sauvegardée) @0x6a7cb2/cb9/cbd
            ...
            case 5: { // boîte, position uniforme dans [-box/2,box/2] (@0x6a7d9c)
                p.alive = 1; p.age = 0.0f;
            ...
            case 2: { // DÉGÉNÉRÉ : normalise wA (inutilisé) puis transforme wC STALE via M+20 (@0x6a7b15)
                p.alive = 1; p.age = 0.0f;
                s36C = Math_RandFloatRange(kRandLo, kRandHi);  // wA (calculé mais NON utilisé) @0x6a7b31
                ...
                // (le binaire sauve wC dans var_34C/348/344 @0x6a7b83 — valeurs mortes, non reproduites)
            ...
            case 6: { // DÉGÉNÉRÉ : boîte -> normalise dans wB (wB.z≡wC.x) puis transforme wC via M+20 (@0x6a7e99)
                p.alive = 1; p.age = 0.0f;
            ...
            default:                                           // forme invalide (def_6A7A89) : pas de spawn
                break;
        }
        if (pool->spawnAccum < kSpawnThreshold) break;         // if(spawnAccum<1.0) break @0x6a8001
    }

    // Draine l'excédent d'accumulateur (aucun slot libre restant mais accum>1) @0x6a8028-0x6a8043.
...
//  6 sommets/particule vivante (2 triangles) dans le scratch CPU, quad camera-facing via la base
//  right/up × size, COULEUR GRISE = (u8)size répliqué sur ARGB, DrawPrimitiveUP(TRIANGLELIST, 24).
//
//  ⚠ La couleur du billboard ne provient PAS des champs colorR..A (particle+40..52, intégrés mais
//    NON lus par le rendu) : le binaire lit UNIQUEMENT particle.size (particle+36 = v6 @0x6a710f)
//    et en fait un gris ARGB (v12 = (u8)ftol(size)·0x01010101, @0x6a711d-0x6a7176) qui sert AUSSI
//    de multiplicateur de coin. C'est fidèle : l'intensité et la taille « fondent » ensemble via size.
//
//  ⚠ Les UV ne sont PAS écrites par 0x6A70B0 (pré-initialisées une fois dans le VB partagé). Notre
//    scratch étant reconstruit chaque frame, on pose les UV standard dans l'ordre des coins.
// ===========================================================================================
int ZoneFx_RenderBillboards(FxParticlePool* pool, const ZoneFxFrameParams& params) {
    ...
    // Cull frustum de l'origine (Cam_FrustumTestPoint6(g_GfxRenderer, this+12) @0x6a70d8).
    ...
    for (int i = 0; i < pool->particleCount; ++i) {           // v22 < particleCount @0x6a7437
        ...
        // Couleur GRISE : c = (u8)ftol(size) ; ARGB = c·0x01010101 (@0x6a711d-0x6a7176).
        const DWORD c = static_cast<DWORD>(Crt_ftol(s) & 0xFF);
        const D3DCOLOR col = c | (c << 8) | (c << 16) | (c << 24);
        // Coins camera-facing (base × size), + position de la particule (particle+8/+12/+16).
        ...
        ++quads;                                               // ++v15 @0x6a741d
        if (quads == params.maxQuads) break;                   // if(v15==dword_7FFEE0) break @0x6a7424 (0 => jamais)
    }
