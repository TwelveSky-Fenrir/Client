// Gfx/ParticleSystem.cpp — implémentation FIDÈLE du pool de particules GXD.
//
// Vérité IDA : TwelveSky2.exe. Chaque bloc cite son ancre (nom + 0xADDR).
// Voir ParticleSystem.h pour la carte des structures et Docs/TS2_FX_ROSETTA.md §2.
...
// π/180 — constante littérale du binaire (0.01745329238474369) utilisée par
// PtclDef_UpdateAndSpawn 0x423688/0x4236AA/0x4236CC (degrés → radians).
static constexpr float kDeg2Rad = 0.01745329238474369f;

// ±100 — bornes de direction aléatoire (flt_7EDB54 = 100.0 / flt_7EDB50 = -100.0),
// chargées en tête de boucle de spawn @0x42376E/0x423776.
static constexpr float kRandHi = 100.0f;
static constexpr float kRandLo = -100.0f;

// ---------------------------------------------------------------------------
// VB partagé — g_GxdRenderer_pSpritePool 0x18C5110. Sert de gate global :
// AllocPool/Update/Render ne font rien tant qu'il est nul (mêmes tests que le
// binaire @0x423291 / 0x42332F / 0x42445A).
...
// ---------------------------------------------------------------------------
// RNG — Rng_Next 0x7603FD (LCG MSVC). Le binaire lit l'état rand par-thread du
// CRT (Crt_GetPtd[5]) ; on reproduit l'ALGORITHME avec un état de module.
static unsigned int s_rngState = 1;

void SetRandSeed(unsigned int seed) { s_rngState = seed; }

unsigned int Rng_Next() {
    // state = 214013·state + 2531011 ; renvoie (state>>16)&0x7FFF   (0x76040B..0x76041E)
    s_rngState = 214013u * s_rngState + 2531011u;
    return (s_rngState >> 16) & 0x7FFFu;
}

// Math_RandFloatRange 0x403330 : a + (b-a)·frac ; inverse si b <= a.
...
// ---------------------------------------------------------------------------
// PtclDef_GetTexture — IDirect3DTexture9* stockée à def+0x38 (= texture[0x34]),
// lue par RenderQuads @0x4248F1 pour SetTexture(0, tex).
IDirect3DTexture9* PtclDef_GetTexture(const PtclDef* def) {
    if (!def) return nullptr;
    IDirect3DTexture9* tex = nullptr;
    // def+0x38 : la cTexture (56 o @0x04) range son pointeur GPU à son offset +0x34.
    std::memcpy(&tex, def->texture + 0x34, sizeof(tex));
    return tex;
}

// ---------------------------------------------------------------------------
// PtclDef_Init 0x4221E0 / PtclDef_Reset 0x4222D0 — zéro-init. Le seul champ non
// nul est le handle de texture (dword idx 11 = def+0x2C) posé à -1.
...
// ---------------------------------------------------------------------------
// PtclDef_ClampParams 0x422F60 — borne les scalaires puis DÉRIVE colorRate.
...
// ---------------------------------------------------------------------------
// PtclDef_AllocPool 0x423280 — dimensionne le pool (count = ftol(lifetime·spawnRate)).
void PtclDef_AllocPool(PtclDef* def, PtclPool* pool) {
    ...
    for (int i = 0; i < count; ++i)             // 0x4232E6..0x423301 : chaque particle.alive = 0
        buf[i].alive = 0;
...
// ---------------------------------------------------------------------------
// PtclDef_FreePool 0x423240 — libère le tableau et remet scale = 1.
...
// ---------------------------------------------------------------------------
// Spawn d'UNE particule (corps du switch de forme dans PtclDef_UpdateAndSpawn).
// `W` = matrice monde du pool (rot XYZ · trans). Reproduit la sémantique des 10
// cas @0x4237AD ; les cas où le désassemblage aliase des slots de pile (1/6/7/10)
// sont reconstruits d'après le motif confirmé « scalaire·direction normalisée »
// partagé avec les cas 2/3/4/5/8/9 (voir §3-B ; index d'IDA, JAMAIS VeryOld).
static void Fx_SpawnOne(Particle& p, const PtclDef* def, const D3DXMATRIX& W) {
    ...
    D3DXVECTOR3 lp(0, 0, 0);   // position locale (transformée en COORD)
    D3DXVECTOR3 lv(0, 0, 0);   // vitesse locale (transformée en NORMAL)

    // rand par-axe des bornes de vitesse (motif LABEL_31/32/37 : velMin/velMax XYZ)
    auto velPerAxis = [&]() {
    ...
    switch (shape) {
        case 1: {   // point — loc_4237B4
            // dir3 = normalize(rand[-100,100]³) ; lp = baseSpeed·dir3 ; vel par-axe.
            // NB : le décompilé aliase le slot de dir @0x42382D (lit baseSpeed·slot
            // périmé) ; reconstruit d'après les cas frères 2/3 (scalaire·dir).
        ...
        case 3: {   // sphère — loc_423A8A
            // lp = rand(0,baseSpeed)·dir3 ; vel par-axe.
            D3DXVECTOR3 d(Math_RandFloatRange(kRandLo, kRandHi),
                          Math_RandFloatRange(kRandLo, kRandHi),
                          Math_RandFloatRange(kRandLo, kRandHi));
        ...
        case 5: {   // boîte (volume) — loc_423D54
            // lp = rand(±0.5·boxSize) par-axe ; vel par-axe.
        ...
        case 6: {   // anneau — loc_423E04
            // dir3 = normalize(rand ±0.5·boxSize) ; lp = dir3 (unité) ;
            // lv = rand(velMin0,velMax0)·dir3.  (slots vélocité aliasés → motif frère.)
        ...
        // Cas 7..10 : émission PLANAIRE 2D. dir2 = normalize2D(rand[-100,100]²) mappée
        // sur le plan X-Z ; la composante Y = rand(±0.5·boxSize1) (hauteur de boîte).
        // Motif confirmé par le cas 9 (le plus lisible) ; 7/10 aliasent le slot dir2.
        case 7: {   // planaire — loc_423F19
        ...
        default:
            return;                                                // def_4237AD : forme inconnue
    }
    ...
    // color = startColor (LABEL_34 @0x423906 : recopie de def+0x90..0x9C)
...
// ---------------------------------------------------------------------------
// PtclDef_UpdateAndSpawn 0x423310 — une frame de simulation + émission.
void PtclDef_UpdateAndSpawn(PtclPool* pool, const float position[3],
    ...
    // --- Intégration des particules vivantes (boucle 0x4233F2..0x423565) ---
    int aliveCount = 0;                                          // v4
    ...
    for (int i = 0; i < cap; ++i) {
        ...
        if (0.0f < p.param0) {                                  // 0x423476 : garde 1/param0
    ...
    // --- Gate d'émission (0x423597) : v34 = 0.0 constant, def+0x4C = field19,
    //     pool+0x14 = elapsedTime. Émet tant que field19 ≤ 0 OU field19 ≥ elapsedTime. ---
    if (0.0f >= def->field19 || def->field19 >= pool->elapsedTime) {
        ...
        if (pool->spawnAccum >= 1.0f) {                        // 0x4235DA
            // Matrice monde du pool : rot XYZ (degrés) · translation position.
            // (Construction 0x423672..0x423749 : D3DXMatrixTranslation + RotationX/Y/Z
            //  + 4× D3DXMatrixMultiply. L'ordre exact des produits et un éventuel
            //  facteur « motion » @def+0x3C ne sont pas prouvables au décompilé —
            //  facteur motion = identité ici (cas sans MOTION). TODO ancré 0x4235E0.)
            ...
            // Boucle de spawn : remplit les slots morts jusqu'à drainer spawnAccum
            // ou épuiser la capacité (0x42378B..0x4243B2).
            for (int i = 0; i < cap; ++i) {
                Particle& p = pool->particles[i];
            ...
            // Draine le reliquat non émis (capacité pleine) — 0x4243D0..0x4243EA.
            if (pool->spawnAccum > 1.0f) {
                do { pool->spawnAccum -= 1.0f; } while (1.0f < pool->spawnAccum);
            }
        }
    } else if (aliveCount == 0) {                               // 0x42359F : fenêtre finie + pool vide
        PtclDef_FreePool(pool);                                // 0x4235A7 : auto-libération
    }
}

// ---------------------------------------------------------------------------
// Empaquetage couleur ARGB depuis les floats 0..255 (0x424566..0x424605) :
//   D3DCOLOR = A<<24 | R<<16 | G<<8 | B  (troncation float→int puis octet).
...
// ---------------------------------------------------------------------------
// InitSharedQuadUVs — pré-remplit les uv (+0x10) des sommets du VB partagé.
// RenderQuads n'écrit QUE pos+color ; l'ordre des 6 sommets (2 triangles) est
// TL, TR, BL / BL, TR, BR (voir écritures 0x4246CB..0x424895).
...
// ---------------------------------------------------------------------------
// PtclDef_RenderQuads 0x424430 — construit les quads billboard dans le VB
// partagé et lance le DrawPrimitiveUP.
void PtclDef_RenderQuads(PtclPool* pool, const ParticleFrameParams& params) {
    // Gates 0x42445A : pool initialisé + VB partagé + origine dans le frustum.
    ...
    // Fondu LOD par distance (0x424481..0x4244E6) :
    ...
    // Compte les particules vivantes (0x4244FB..0x42450E).
    int aliveCount = 0;
    for (int i = 0; i < cap; ++i)
        if (pool->particles[i].alive) ++aliveCount;

    // Nombre de quads réellement dessinés = ftol(alive · fade)  (0x42451A).
    ...
    for (int i = 0; i < cap; ++i) {                           // 0x42453C..0x4248D0
        Particle& p = pool->particles[i];
        if (!p.alive) continue;
        if (quads > drawCap - 1) continue;                    // 0x42454D : plafond LOD

        float size = p.size;                                  // v29 : particle+0x24
        D3DCOLOR col = Fx_PackColor(p);                       // v16

        // Coins billboard = camRight/camUp × demi-taille (0x424617..0x4246BE).
    ...
    if (quads) {                                              // 0x4248D8
        // SetTexture(0, def texture) puis DrawPrimitiveUP(TRIANGLELIST, 2·quads, VB, 24).
...
// ---------------------------------------------------------------------------
// I/O — hooks injectables pour texture/motion (sous-systèmes tiers non possédés).
...
// Lit `size` octets dans `dst` depuis `hFile` ; renvoie true si tout est lu
// (mimique ReadFile + contrôle du compte, comme chaque étape de 0x422C50).
...
bool PtclDef_ReadFile(PtclDef* def, HANDLE hFile, int a3, int a4) {
    ...
    // enabled (4 o) — si 0 sur disque, def vide acceptée telle quelle.
    if (!Fx_ReadExact(hFile, &def->enabled, 4)) return false;  // 0x422C7D
    if (!def->enabled) return true;                            // 0x422C8C : def désactivée
    def->enabled = 0;                                          // 0x422CAB : remis à 0 le temps du chargement
    ...
    // Bloc numérique séquentiel (offsets/ordre EXACTS de 0x422C50).
    const bool ok =
