// Game/ItemPickupSystem.cpp — voir ItemPickupSystem.h pour les references EA detaillees.
...
// Distance euclidienne 3D (equivalent Math_Dist3D, EA 0x53faa0 : sqrt(dx^2+dy^2+dz^2)).
...
GroundPickupTarget FindNearestGroundItem(GameWorld& world) {
    ...
    // Position du joueur local, equivalent de flt_1687330 (bloc "self" de l'entite index 0
    // du tableau joueurs, cf. GameState.h PlayerEntity::x/y/z).
...
bool IsWithinPickupRange(const GameWorld& world, const GroundItem& target) {
    ...
    // world.Self() n'est pas const (peut inserer un joueur par defaut) ; on accede ici en
    // lecture seule via une reference non-const locale, coherent avec l'usage de ce systeme
    // (appele apres FindNearestGroundItem, qui garantit deja un joueur present).
    ...
    // if (Math_Dist3D(cible, joueur) <= 20.0)
...
PickupOutcome EvaluatePickup(GameWorld& world, GroundPickupTarget& outTarget,
    ...
    // TODO (EA 0x5db530) : emission de la requete reseau de confirmation de ramassage —
    // non identifiee dans Item_PickupTarget/Item_InteractGround (voir commentaire du .h).
}

int32_t ValidateSplitQuantity(int32_t maxDisponible, int32_t quantiteDemandee,
                               bool aSaisieUtilisateur) {
    // EA 0x5b1713/0x5b1731 (cas 1, motif identique dans tous les autres cas du switch) :
    ...
    // EA 0x5b178c : if (v >= 1) { ... } else { sub_5B02D0(this) /* annulation */ }
    if (quantite < 1)
        return 0;
...
int32_t ValidateSplitQuantity(const InvCell& cellule, int32_t quantiteDemandee,
                               bool aSaisieUtilisateur) {
    // cas this[4]==1 : g_InvGrid_Count[384*row+6*col] -> InvCell::flag (cf. ClientRuntime.h).
...
int32_t ValidateSplitQuantity(const GroundPickupSlot& cellule, int32_t quantiteDemandee,
                               bool aSaisieUtilisateur) {
    // cas this[4]==5 : dword_1674400[42*conteneur+3*slot] -> GroundPickupSlot::count.
