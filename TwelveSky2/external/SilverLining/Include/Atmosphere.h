 ...
 */
void GetHorizonColor(float yawDegrees, float pitchDegrees, float *r, float *g, float *b) const;
...
/** Returns the average color of the sky at the horizon. Based on the current camera orientation
   ...
   \param b A pointer to a float to receive the red channel of the horizon color, from 0 - 1.0
 */
void GetHorizonColor(float pitchDegrees, float *r, float *g, float *b) const;
...
/** Call this immediately after constructing your scene's Atmosphere and initializing your
   graphics subsystem (OpenGL, OpenGL32, DirectX9, DirectX10, or DirectX11). This method will configure SilverLining to use
   ...
   \param renderer Pass the enumerated constant \c SilverLining::OPENGL or \c SilverLining::OPENGL32CORE or
     \c SilverLining::DIRECTX9 or \c SilverLining::DIRECTX10 or
                    \c SilverLining::DIRECTX11 or \c SilverLining::CUSTOM_RENDERER
   ...
   \param environment Only required for DIRECTX9, DIRECTX10, or DIRECTX11 renderers; a pointer to your
...
void SetCameraMatrix(const double *cameraMatrix);
...
/** Call this at the beginning of your rendering loop. At the start of each frame in your scene,
   ...
   horizon coordinates.
    ...
    (1000)
    \param drawStars Set true to draw the stars when the sky is dark, conditions are not
    overcast, and fog is not present - or false to disable the drawing of the stars under
    all circumstances.
...
/** Call this at the end of your rendering loop. After drawing all of your scene's objects, call
   ...
   actually draw them; this list may be obtained with GetObjects().
    ...
    You may want to do this if you are calling EndFrame() multiple times per frame (say, for
    multiple viewports or rendering passes with different viewpoints.) Otherwise, the internally
    maintained "last position" of each precipitation particle will be incorrect and rain particles
    will render incorrectly; you should only call EndFrame with drawPrecipitation set to true once
    per frame. An alternate solution to this problem is to maintain a seperate Atmosphere object
    per view.

   \return true if the clouds were successfully drawn.
 */
bool EndFrame(bool drawClouds = true, bool drawPrecipitation = true);
...
/** Allows you to disable culling of clouds against the far clip plane of your projection matrix.
   ...
   need to disable clipping explicitly prior to calling Atmosphere::EndFrame() for this call to have an
   effect. In OpenGL, you can do this using glEnable(GL_DEPTH_CLAMP_NV) in conjuction with
   glDepthFunc(GL_LEQUAL). */
void DisableFarCulling(bool bDisable) {
 ...
 */
static void EnableHDR(bool hdr) {
    enableHDR = hdr;
}
...
/** Overrides the default FILE* - based resource loader with a customer-supplied resource loader object.
   ...
   initializing your first Atmosphere object, but before calling Atmosphere::Initialize(). */
...
Color Interpolate(const Color& over, const Color& under) const;
};
}
